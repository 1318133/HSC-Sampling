...
class ResBlock_changemarx(nn.Module):
    ...
    #     HWC = self.HWC_dwconv(self.HWC(x))
    #     HW,CW,HC = HWC.chunk(3, dim=1) 
    #     x_sobel_HW = self.sobel_c(x)#Sobel_operation(x)
    #     x_sobel_HW_a = x_sobel_HW*HW
    #     x_sobel_CW = self.sobel_c(x.permute(0, 2, 1, 3))#Sobel_operation(x.permute(0, 2, 1, 3))  # 将特征张量转置为目标形状 (B, H, C, W)
    #     CW = CW.permute(0, 2, 1, 3)
    #     x_sobel_CW_a = x_sobel_CW*CW
    #     x_sobel_CW_a = x_sobel_CW_a.permute(0, 2, 1, 3)
    #     x_sobel_HC = self.sobel_c(x.permute(0, 3, 2, 1))#Sobel_operation(x.permute(0, 3, 2, 1))  # 将特征张量转置为目标形状 (B, W, H, C)
    #     HC = HC.permute(0, 3, 2, 1)
    #     x_sobel_HC_a = x_sobel_HC*HC
    #     x_sobel_HC_a = x_sobel_HC_a.permute(0, 3, 2, 1)
    #     x_sobel_a = self.Sobel_conv(torch.cat((x_sobel_HW_a,x_sobel_CW_a,x_sobel_HC_a),dim=1))
    #     out = x_sobel_a#*x+out#self.cat_conv(torch.cat((x_sobel_a,out),dim=1))
...
class crosslearn(nn.Module):
    def __init__(self, channel):
        super(crosslearn, self).__init__()
        self.c1=ResCA(48)
        self.c2 = ResCA(48)
        self.c3 = ResCA1(48)
        ...
        self.q1 = nn.Sequential( nn.Conv2d(144, 48, 1, 1, 0, bias=False))
        self.q2 = nn.Sequential(nn.Conv2d(144, 48, 1, 1, 0, bias=False))
        self.q3 = nn.Sequential(nn.Conv2d(144, 48, 1, 1, 0, bias=False))
        self.q4 = nn.Sequential(nn.Conv2d(144, 48, 1, 1, 0, bias=False))
        self.q5 = nn.Sequential(nn.Conv2d(144, 48, 1, 1, 0, bias=False))
        self.q6 = nn.Sequential(nn.Conv2d(144, 48, 1, 1, 0, bias=False))



        self.sq1=nn.Sequential(nn.Conv2d(96, 96, 3, 1, 1, bias=False),
                               nn.Conv2d(96, 48, 1, 1, 0, bias=False))

        self.sq2 = nn.Sequential(nn.Conv2d(96, 96, 3, 1, 1, bias=False),
                                 nn.Conv2d(96, 48, 1, 1, 0, bias=False))
        self.sq3 = nn.Sequential(nn.Conv2d(96, 96, 3, 1, 1, bias=False),
                                 nn.Conv2d(96, 48, 1, 1, 0, bias=False))
        self.sq4 = nn.Sequential(nn.Conv2d(96, 96, 3, 1, 1, bias=False),
                                 nn.Conv2d(96, 48, 1, 1, 0, bias=False))
        self.sq5 = nn.Sequential(nn.Conv2d(96, 96, 3, 1, 1, bias=False),
                                 nn.Conv2d(96, 48, 1, 1, 0, bias=False))
        self.sq6= nn.Sequential(nn.Conv2d(96, 96, 3, 1, 1, bias=False),
                                 nn.Conv2d(96, 48, 1, 1, 0, bias=False))
        self.sq7 = nn.Sequential(nn.Conv2d(96, 96, 3, 1, 1, bias=False),
                                 nn.Conv2d(96, 48, 1, 1, 0, bias=False))
        self.sq8 = nn.Sequential(nn.Conv2d(96, 96, 3, 1, 1, bias=False),
                                 nn.Conv2d(96, 48, 1, 1, 0, bias=False))

        self.conv1 = nn.Conv2d(48, 31, 3, 1, 1)
        self.conv2 = nn.Conv2d(48, 31, 3, 1, 1)
        self.conv3 = nn.Conv2d(48, 31, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        z1=self.sq1(torch.cat((x3,y3),dim=1))
        z11=self.cs1(z1)
        z110= self.q1(torch.cat((z11,x1,y1),dim=1))
        z12 = self.cs2(z110)
        z120 = self.q2(torch.cat((z12, x2, y2), dim=1))
        z13 = self.cs3(z120)+x
        z130 = self.q3(torch.cat((z13, x3, y3), dim=1))



        x11=self.c1(z1)
        x12=self.sq2(torch.cat((x11,z110),dim=1))
        x13=self.c2(x12)
        x14=self.sq3(torch.cat((x13,z120),dim=1))
        x15=self.c3(x14)
        x16=self.sq4(torch.cat((x15,z130),dim=1))
        x17=self.c4(x16)+x

        y11=self.s1(z1)
        y12=self.sq5(torch.cat((y11,z110),dim=1))
        y13=self.s2(y12)
        y14=self.sq6(torch.cat((y13,z120),dim=1))
        y15=self.s3(y14)
        y16=self.sq7(torch.cat((y15,z130),dim=1))
        y17=self.s4(y16)+x

        z2 = self.sq8(torch.cat((x17, y17), dim=1))
        z21=self.cs1(z2)
        z21 = self.q4(torch.cat((z21, x11, y11), dim=1))
        z22 = self.cs2(z21)
        z22 = self.q5(torch.cat((z22, x13, y13), dim=1))
        z23 = self.cs3(z22)
        z23 = self.q6(torch.cat((z23, x15, y15), dim=1))
        z24 = self.cs4(z23)
...
#                                   nn.PixelUnshuffle(2))#nn.PixelUnshuffle
